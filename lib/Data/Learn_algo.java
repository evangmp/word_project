...
public class Learn_algo {
    ...
    public Learn_algo() {} // IDK, but it works

    public void learn_algo_center() { // oh
        ...
        if (gap > 3) { // the last words with a low number of training sessions
        ...
        int comp = 0;
        if(sorted.size() < 11) { // complete with words having the lowest level
        ...
        String choice;
        for (int i = 0; i < fr_word_l.size(); i++) { // print and test
    ...
    public Map<Integer, Integer> sort(Map<Integer, Integer> base, Map<Integer, Integer> sorted) {
        ...
        for (int i = 0; i < base.size()+sorted.size(); i++) {
            if(!Objects.equals(base.get(i), null)) { // find the lowest
                if (base.get(i) < min) {
                    min = base.get(i);
                }}
        }
...
class DB_com extends Learn_algo{ // connection with DB
    ...
    public void dbc_lvl(String change, String fr_word) {
        try {
